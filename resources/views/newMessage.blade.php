   ...
   <li @click="friendID({{$friend->id}})" v-on:click="seen = true" style="list-style:none;
    margin-top:10px; background-color:#F3F3F3" class="row">
      ...
      <div class="col-md-3 pull-left">
           <img src="{{Config::get('app.URL')}}/image/{{$friend->pic}}"
         style="width:50px; border-radius:100%; margin:5px">
       </div>

       <!-- "'{{Config::get('app.URL')}}/image/' + singleMsg.pic" -->

      <div class="col-md-9 pull-left" style="margin-top:5px">
        <b style="margin-left:10px"> {{$friend->username}}</b><br>
...
<p class="alert alert-success">@{{msg}}</p>
   ...
   <div  v-if="seen">
      ...
      <input type="button" class="btn btn-sm btn-outline-dark" value="send message" @click="sendNewMsg()">
  </div>
